<div class="bg-gray-100 w-64 min-h-screen flex flex-col border-r border-gray-400">
    ...
    <nav class="flex flex-col gap-4 p-6">
        ...
        <a href="{{ route('dashboard.formations.index', ['groupId' => $group->id]) }}" class="flex gap-[10px] border border-transparent hover:bg-gray-300 hover:border-gray-400 p-2 rounded transition"><img src="{{ asset('images/icons/Book-open.svg') }}" alt="formações">Formações</a>
        <a href="{{ route('events.index', ['groupId' => $group->id]) }}" class="flex gap-[10px] border border-transparent hover:bg-gray-300 hover:border-gray-400 p-2 rounded transition"><img src="{{ asset('images/icons/Clock.svg') }}" class="w-5" alt="galeria">Eventos</a>
